-- Hub Script v1.1 | ESP + AutoHop + AutoChest + AutoFruit
...
Main.Name             = "Main"
Main.Size             = UDim2.new(0, 230, 0, 365)
Main.Position         = UDim2.new(0.5, -115, 0.5, -182)
...
TitleLabel.Text                 = "Hub Script v1.1"
...
local function hopServer()
    ...
    for _, s in ipairs(servers) do
        if s.id ~= cur and (s.playing or 0) >= 1 then picked = s break end
    end
    if not picked then
        for _, s in ipairs(servers) do
...
local function nameContains(obj, keywords)
...
local function fireRemotes(obj)
...
local function findObjects(keywords)
    ...
    local function scan(parent)
        for _, obj in ipairs(parent:GetChildren()) do
            if (obj:IsA("Model") or obj:IsA("BasePart")) and nameContains(obj, keywords) then
                local root = obj:IsA("Model")
                    and (obj.PrimaryPart or obj:FindFirstChildWhichIsA("BasePart"))
                    or obj
                if root then table.insert(results, {obj=obj, pos=root.Position}) end
            end
            scan(obj)
        end
    end
...
local function runAutoChest()
    while State.AutoChest do
        local chests = findObjects(CHEST_KEYWORDS)
        if #chests == 0 then
            ...
            for i, c in ipairs(chests) do
                ...
                setStatus(string.format("Chest %d/%d", i, #chests), Color3.fromRGB(100,180,255))
                tpTo(c.pos)
                task.wait(0.6)
                fireRemotes(c.obj)
                task.wait(0.6)
            ...
            if State.AutoChest then
                setStatus("Chests done! Hopping...", Color3.fromRGB(255,200,50))
                task.wait(1.5)
                hopServer()
                task.wait(5)
            end
...
-- │      AUTO FRUIT         │
-- └─────────────────────────┘
-- Nama-nama fruit di Blox Fruits
local FRUIT_NAMES = {
    "fruit", "devil",
    "flame","ice","sand","dark","light","magma","quake",
    "human","buddha","love","spider","bird","bomb","spike",
    "chop","smoke","string","revive","door","paw","gravity",
    "diamond","rubber","barrier","ghost","dough","shadow",
    "leopard","control","venom","soul","phoenix","dragon",
    "mammoth","sound","blizzard","rumble","pain","kitsune",
    "toad","gas","t-rex","portal","winter","spring"
}
...
local function collectFruit(obj)
    ...
    -- Coba ProximityPrompt (cara paling reliable)
    ...
    -- Fire semua remote yang ada
    fireRemotes(obj)
end

local fruitSpawnConn

local function runAutoFruit()
    -- Scan buah yang udah ada di workspace
    local function scanFruits()
        for _, obj in ipairs(workspace:GetChildren()) do
            if not State.AutoFruit then return end
            if (obj:IsA("Model") or obj:IsA("BasePart")) and nameContains(obj, FRUIT_NAMES) then
                setStatus("Fruit: " .. obj.Name, Color3.fromRGB(255, 220, 50))
                collectFruit(obj)
                task.wait(1)
            end
        end
    ...
    -- Listen real-time spawn
    fruitSpawnConn = workspace.ChildAdded:Connect(function(child)
        if not State.AutoFruit then return end
        task.wait(0.8) -- tunggu object fully loaded
        if (child:IsA("Model") or child:IsA("BasePart")) and nameContains(child, FRUIT_NAMES) then
            setStatus("Fruit spawned: " .. child.Name, Color3.fromRGB(255, 220, 50))
            task.wait(0.5)
            collectFruit(child)
        end
    end)
    ...
    -- Periodic rescan tiap 15 detik (jaga-jaga kalau ada yang kelewat)
    while State.AutoFruit do
        setStatus("AutoFruit: Watching...", Color3.fromRGB(160, 160, 255))
        task.wait(15)
        if State.AutoFruit then scanFruits() end
    end

    if fruitSpawnConn then
        fruitSpawnConn:Disconnect()
        fruitSpawnConn = nil
    end
end
...
createToggle("Auto Collect Chest", "Kumpul chest → auto hop", 3, function(on)
    State.AutoChest = on
...
createToggle("Auto Fruit", "Detect & collect devil fruit spawn", 4, function(on)
    State.AutoFruit = on
    if on then
    ...
    setStatus(on and "AutoFruit ON" or "AutoFruit OFF", on and Color3.fromRGB(255,220,50) or Color3.fromRGB(200,80,80))
end)

setStatus("Hub v1.1 loaded! Tap toggle untuk aktifin")
print("[Hub] v1.1 Loaded! ESP + AutoHop + AutoChest + AutoFruit")
